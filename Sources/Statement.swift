...
extension StatementList {
	...
	public func execute(parameters ps:[(name:String, value:@autoclosure()->Value)]) -> Execution {
...
extension Statement {
	...
	func bind2(parameters ps:[(name:String, value:@autoclosure()->Value)]) {
		for i in 0..<ps.count {
			let	v			=	ps[i].value()
...
//	func bind(parameters ps:[String:Value]) {
////		}
////		for (k, v) in ps {
////			
